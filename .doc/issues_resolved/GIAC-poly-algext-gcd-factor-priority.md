...
**状态:** **resolved**（2026-06-24；T0–T3 全部验收）  
**类型:** 实施计划 / AFK（已归档）  
**父项:** [GIAC-poly-algext-backlog](../issues/GIAC-poly-algext-backlog.md) §6 **P3-1…P3-3**、§7 **P4-***  
**相关:** [GIAC-algext-adoption](../issues/GIAC-algext-adoption.md) §8.4、[GIAC-poly-p3-6-quartic-roots-gaps](../issues/GIAC-poly-p3-6-quartic-roots-gaps.md)（P3-6 ✅）、[expr-poly-conversion.md](../expr-poly-conversion.md)、[giac-poly-api-stability.md](../giac-poly-api-stability.md)  
...
**快照:** 2026-06-24（**T0-1…T0-3 ✅**、**T1-1…T1-3 ✅**、**T2-1…T2-4 ✅**、**T3-1…T3-3 ✅**；复核 2026-06-24）

---

## 1. 问题陈述（已闭环）

原缺口（2026-06 前）：`Poly<AlgExtC>` 表示层（P1）与 deg≤4 求根（P3-6）已落地，但 **K 上 gcd/factor 主路径缺**，导致：

- `factor(x²−2)`、`gcd(x²−2, x−√2)` 无法走多项式管线
- `partfrac(1/(x²−2), x)` 卡在 disc>0 二次分裂
- `solve` / `integrate` 仍依赖 `giac-simplify` / `giac-solve` 的 rootof 形状特判

**现状（T0–T3 已落地）：** 上述三条主路径已走 `giac-core` K 上 gcd/factor；`try_factor_quadratic_sqrt` 已删；`partfrac(1/(x²−2),x)` 绿。**遗留（本 issue 范围外）：** `eval(factor(x²−2))` 在 ℚ 上仍保持不可约形态（`B-FACTOR-ALGEXT` ignored，见 [GIAC-expr-api-test-contains-cleanup](../issues/GIAC-expr-api-test-contains-cleanup.md)）；K 上 Zassenhaus 子集 **不做**（§5）。
...
| **明确不做** | — | `_EXT` 系数 **无** Hensel / sparse_bi / unitaryfactor（与 [GIAC-poly-algext-backlog](../issues/GIAC-poly-algext-backlog.md) §6 一致） |
...
| 本 issue | [GIAC-poly-algext-backlog](../issues/GIAC-poly-algext-backlog.md) |
|----------|-----------------------------------------------------------|
| T0-* | P0-D 收尾、P1 表示层契约 |
| T1-* | **P3-1**、**P3-2** |
...
4. **用 `Poly::div_rem`（多元 leading）验 K 上一元整除** — 须 `quo_exact_wrt` / `FlatUni<C>`（见 [GIAC-poly-nested-ring-types](../issues/GIAC-poly-nested-ring-types.md)）。
...
cargo test -p giac-poly --lib gcd_field

# 波次 2 起
cargo test -p giac-core gcd_
cargo test -p giac-core factor_

# 波次 3 跨 crate
cargo test -p giac-core partfrac
cargo test -p giac-solve solve_reducible_deg5
cargo test -p giac-simplify eval_factor
cargo test -p giac-core --test partfrac_k_route
```

| 波次 | 硬门禁 | 复核 |
|------|--------|------|
| 1 | `rem(x²−2, x−√2)=0`；`gcd(x²−2, x−√2)=x−√2` | ✅ `poly_alg_ops` gcd/rem 测绿 |
| 2 | `factor(x²−2)` 乘积 = 原式；`factor(x⁴−4)` 分裂 | ✅ `poly_alg_factor` 7 测绿 |
| 3 | `partfrac(1/(x²−2),x)`；solve 降次五根 | ✅ `partfrac_k_route` + `solve_reducible_deg5_product` 绿 |
| — | `eval(factor(x²−2))` expand 分裂 | ⏸ **不做** — ℚ 上保持 `x²−2`（`B-FACTOR-ALGEXT`）；K 分裂经 `factor_into_algext` / solve 路径验收 |
