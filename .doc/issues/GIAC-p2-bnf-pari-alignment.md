...
| **P2 ◐** | `unit_group` 完整化 | 中（`unit_group.rs` 已有骨架，补 LLL 搜索） | `unit_group.rs::fundamental_units` 补实二次 `d≥3` LLL 搜索；`fundamental_unit_via_cf_sqrt_d`（Pell/CF，大 regulator `d≡2,3mod4`）；`bnfregulator`/`bnfisunit`/`bnfunits` | `bnfunits(P)`✅(2b-S1 实二次r=1 + 大regulator CF兜底) / `bnfregulator(P)`✅(2b-S1 + √163 R≈18.669)；Dirichlet 基单位(2b-S2 r≥2)、regulator(2b-S2 多单位)、单位群离散对数、`d≡1mod4` 大regulator |
...
- **2a-S2b-full 主体 ✅ 关系查找 + 完备性认证（基础设施完备；DIV-100、DIV-101 均已修，虚二次 ℚ(√-14)、ℚ(√-23) Buchmann 认证解锁）**：
  ...
  - `ponytail:` 全格关系查找 + ramification + SNF + `_full` 赋值 + 解析类数公式 = **完备基础设施 + 实二次 h>1 证书**，DIV-100/DIV-101 均已修。**实二次大 regulator 已解锁**：`unit_group.rs::fundamental_unit_via_cf_sqrt_d(d)` —— `√d` 连分数首 Pell 解 `p²−dq²=±1` = 基本单位（BigInt 渐近分数，坐标可超 i128），`fundamental_units` 暴力 256-bound 找不到时对 `𝓞_K=ℤ[√d]`（`d≡2,3 mod4`）兜底；**解锁 `class_number(ℚ(√163))=1`**（ε~1.27×10⁸，R≈18.669），`bnfregulator`/`bnfunits` 同步受益。仍待：① `d≡1 mod4` 大 regulator（半整数单位 `x²−dy²=±4`，需 `(1+√d)/2` 的 CF / Shanks infrastructure）；② `class_group(P)`/`is_principal(ideal)`/2c。
