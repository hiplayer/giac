...
| **partfrac 高次** | 不可约次数 >3、实二次 disc>0 未分裂 | **Partial** | deg≤3 不可约 ✅；`disc>0` → `NotImplemented("partfrac real quadratic split")` → [GIAC-poly-algext-backlog](GIAC-poly-algext-backlog.md) **P2-3/P2-4** |
...
| 实二次 `disc > 0` 且 factor 不分裂 | `NotImplemented` → **P2-3**（需 `Poly<AlgExtC>`） |
| 线性 / 重根 / `disc≤0` 二次 | ✅ |
| 仿射幂次线性方程组 | ✅（次数 ≤3） |
...
```text
P0  partfrac disc>0 — AlgExt 接入（见 GIAC-poly-algext-backlog P2-3）
