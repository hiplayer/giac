...
**状态:** open（Phase A 已落地 2026-06-30；Phase B 已落地 2026-06-30（good-prime 路径完整：inert + split-prime Hensel）；Phase C C4 已落地 2026-06-30（tame **与** wild bad-prime Dedekind-Kummer，`p ∤ index`；wild 经 char-p p-次根抽取）；p-adic 算术已抽为 `algebra/padic.rs` 稳定子模块；`p | index`（Montes/Round 4）/ 单位群 / 大素因子待 Phase C+）
**类型:** 算法升级 / 完备性闭环
...
| **C4** bad prime ramification（p ∣ disc） — **tame + wild 已落地 2026-06-30**（`p ∤ index`：tame 用 SFF 预处理 + Dedekind 判据 + ramified Hensel；wild `(m_α mod p)'=0` 用 char-p p-次根剥离（Frobenius）后走同一路径，`e_i` 乘 `p^k`）；`p ∣ index` 待续（Montes/Round 4） | 补全 (A) | 中大（~200 行，需 Kummer/Dedekind） | C1 |
...
- **明确不实现 / 推迟 Phase C+**：`p | index`（需 Montes/Round 4）、大素因子（`small_primes(60)` 上界外的 `N(u)` 因子）、`v_p(N(u_int)) > PREC_CAP(60)` 的素数（Hensel 代价过高，跳过——sound）。**tame bad-prime（`p ∤ index` 且 `e_i < p`）与 wild bad-prime（`(m_α mod p)'=0` p-次幂）已于 Phase C C4 落地**：tame 用 SFF 预处理 `b = f/gcd(f,f')` 绕过 `factor_mod_irreducibles` 的 Yun 在非平方自由输入上的挂死，重复除法恢复 `e_i`；wild 用 `peel_wild_pth_root` 在 char-p 抽 p-次根（Frobenius `f=g(x^p)=g^p`）剥离 wild 幂到 tame kernel，`e_i` 乘 `p^k`。Dedekind 指数判据 + ramified Hensel 提升 `g_i^{e_i}`。PARI 交叉验证 tame（`m=t⁴+2t²+4` p=3）与 wild（`m=t⁴+2t³+3t²+2t+3` p=2，`e=2,f=2` even）：非平方 `v_𝔭` 奇 + `N` ℚ-平方 ⟹ (A) 拒；真平方 ⟹ (A) 放行。所有跳过都是 negative filter 的 *安全失败*（false-pass，不 false-reject），soundness 由末端 ℚ 验证兜底。详见证明 sketch §5。
