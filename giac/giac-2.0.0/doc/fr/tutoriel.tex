...
\fi
\newenvironment{preuve}{{\bf Preuve~:}}{}
...
\newtheorem{remarque}[prop]{Remarque}


%\theoremstyle{definition}

\newtheorem{defn}[prop]{D\'{e}finition}
\newtheorem{ex}[prop]{Exemple}
\newtheorem{exs}[prop]{Exemples}
\newtheorem{exe}[prop]{Exercice}
...
\ahref{https://xcas.univ-grenoble-alpes.fr}{xcas.univ-grenoble-alpes.fr}
ou utilisable directement en ligne depuis
\ahref{https://xcas.univ-grenoble-alpes.fr/nw/xcasfr.html}{xcas.univ-grenoble-alpes.fr/nw/xcasfr.html}
...
%
\index{integrate@{\verb+integrate+}}
\index{primitive}
...
\index{gaussquad@{\verb+gaussquad+}}
\giacinputmath{
integrate(exp(-x^2));
integrate(exp(-x^2),x,0,10);
evalf(integrate(exp(-x^2),x,0,10));
gaussquad(exp(-x^2),x,0,10);
evalf(sqrt(pi));
}
...
\giacinputmath{solve([x^2+y-2,x+y^2-2],[x,y])}
...
\giacinputmath{odesolve(sin(x*y),[x,y],[0,1],2)}
...
factor(Q);
purge(g);
GF(2,8); // corps fini a 2^8 elements
factor(g^3*x^2+1);
genpoly(5,3,x);
genpoly(2,3,x);
genpoly(2*y+5,3,x);
...
\giacinputmath{
a:='a';
linsolve([x+y+a*z=1,x+a*y+z=1,a*x+y+z=-2],[x,y,z]);
a:=1;
linsolve([x+y+a*z=1,x+a*y+z=1,a*x+y+z=-2],[x,y,z]);
a:=-2;
linsolve([x+y+a*z=1,x+a*y+z=1,a*x+y+z=-2],[x,y,z]);
...
\giacinput{E:=(2*x+1)/(x^2+1);plot(E)}
\giacinput{plot(E,x=-2..2,color=red)}
\giacinput{
couleur(vert);
plot(E,couleur=rouge);
tangent(plot(E),0)
}
\giacinput{
plot([sin(x),x,x-x^3/6],
x=-2..2,couleur=[rouge,bleu,vert])
}
\giacinput{
li:=[(x+k*0.5)^2 for k in range(-5,6)]:;
plot(li,x=-8..8,couleur=range(11))
}
...
$f(x,y)=0$.
\giacinput{plotparam(sin(t)^3+i*cos(t)^3,t,0,2*pi)}
\giacinput{plotparam(t^2+i*t^3,t,-1,1)}
\giacinput{plotpolar(1/(1-2sin(t/2)),t,0,4*pi)}
\giacinput{
gl_x=-7..7; gl_y=-7..7;
plotpolar(tan(t)+tan(t/2),t,0,2*pi,tstep=0.02)
}
\giacinput{plotimplicit(x^2+4*y^2-4,x,y)}
...
\giacinput{scatterplot(lx,ly)}
\giacinput{polygone_ouvert(lx+i*ly)}
%$
...
\giacinput{plotfunc(x^2-y^2,[x,y])}
\giacinput{
plotfunc(x+y^2,[x=-5..5,y=-2..2],
xstep=0.5,ystep=0.1,affichage=vert+rempli)
}
...
\giacinput{plotparam([u,v,u+v],u=-1..1,v=-2..2)}
...
\giacinput{plotparam([u,u^2,u^3],u=-1..1)}
...
\subsection{Syntaxe compatible avec Python}
Comme Python est impos\'e dans l'enseignement secondaire,
Xcas permet maintenant l'\'ecriture de programmes 
avec une syntaxe largement compatible avec Python, ce qui
\'evite d'avoir \`a apprendre un autre langage.

V\'erifiez dans la configuration du CAS (menu Cfg ou directement
affichage dans la barre d'\'etat) que vous \^etes en mode
Xcas Python. Il est conseill\'e de conserver \verb|^| comme
\'equivalent du signe puissance (\verb|^==**|).
Voici par exemple un programme calculant le PGCD de 2 entiers
\'ecrit en syntaxe Python~:
\begin{giacprog}
def pgcdi(a,b):
  while b!=0:
    r=irem(a,b)
  ...
  return a
\end{giacprog}
\giacinput{pgcdi(15,25)}
On a utilis\'e ici l'instruction \verb|irem(a,b)| pour calculer
le reste de la division de $a$ par $b$ plut\^ot que \verb|a % b|
car dans certains modes de Xcas, \verb|a % b| renvoie la classe
de $a$ dans $\mathbb{Z}/b\mathbb{Z}$.
%
...
\giacinput{5000!:;}
\giacinput{product(range(1,5001)):;}
\giacinput{product(cumSum(seq(1,5000))):;}
\giacinput{f:=1; seq(f:=f*n,n=2..5000):;}
