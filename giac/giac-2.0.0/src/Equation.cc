...
namespace xcas {
  ...
  */
...
#if 1// def EMCC2
    Fl_Button * menubar_= new Fl_Button(hscroll->x()+hscroll->w(),hscroll->y(),3*largeur,hscroll->h());
    menubar_->label("Sel.");
    menubar_->callback((Fl_Callback*)cb_Equation_Select);
#else
    Fl_Menu_Bar * menubar_ = new Fl_Menu_Bar(hscroll->x()+hscroll->w(),hscroll->y(),3*largeur,hscroll->h());
    int s= Equation_menu->size();
    Fl_Menu_Item * menuitem = new Fl_Menu_Item[s];
    for (int i=0;i<s;++i)
      *(menuitem+i)=*(Equation_menu+i);
    menubar_->menu (menuitem);
    menubar_->tooltip(gettext("Equation menu"));    
#endif
    menubar=menubar_;
  ...
  Equation::~Equation(){
    ...
    delete vscroll;
    delete hscroll;
    if (Fl_Menu_Bar * menubar_=dynamic_cast<Fl_Menu_Bar *>(menubar))
      delete [] menubar_->menu();
    delete menubar;
  }
