...
or default eval level)"));
  ...
  void Xcas_load_cas_setup(GIAC_CONTEXT) {
    ...
    if (foc && (foc=Fl::focus()->window()) ){
      ...
      if (xcas::Xcas_MainTab){
        ...
        Xcas_Cas_Setup->resize(xcas::Xcas_MainTab->x(),xcas::Xcas_MainTab->y()+40,xcas::Xcas_MainTab->w(),3*foc->h()/4);
      }
      //else
    }
    Xcas_Cas_Setup->show();
  ...
  void Xcas_load_graph_setup(GIAC_CONTEXT) {
    ...
    if (foc && (foc=foc->window())){
      ...
      if (xcas::Xcas_MainTab)
        Xcas_Plot_Setup->resize(xcas::Xcas_MainTab->x(),xcas::Xcas_MainTab->y()+40,xcas::Xcas_MainTab->w(),3*foc->h()/4);
    }
