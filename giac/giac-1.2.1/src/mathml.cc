...
namespace giac {
  ...
  string svg_preamble(double svg_width_cm, double svg_height_cm,double xmin,double xmax,double ymin,double ymax,bool ortho,bool xml){
    ...
    if (ortho){
      ...
      sprintf(pos,"width=\"%.5gcm\" height=\"%.5gcm\" preserveAspectRatio=\"none\" viewBox=\"%.5g %.5g %.5g %.5g\" >\n<g transform=\"translate(0,%.5g) scale(1,-1)\">\n",svg_width_cm+2,svg_height_cm+1,xmin-x_scale,ymin-y_scale,svg_width+3*x_scale,svg_height+2*y_scale,svg_height+2*ymin);
  ...
  string svg_grid(double xmin,double xmax,double ymin,double ymax){
    ...
    double fontscale=x_scale*.5; // (x_scale<y_scale?y_scale:x_scale)*.3;
    char buffer[16384];
    char * pos=buffer;
    ...
    for (i=(int) i_min_x;  i*dx<=xmax ; ++i){
      x=i*dx;
      sprintf(pos,"<text x=\"%.5g\" y=\"%.5g\" transform=\"scale(1,%.5g)\" style=\"font-size:%.5gpt; text-anchor:middle;\">%.5g</text>\n",x,(ymax+0.6*y_scale)/ratio,ratio,fontscale,x);
      pos = buffer+strlen(buffer);
    ...
    for (i=(int) i_min_y;  i*dy<=ymax ; ++i){
      y=i*dy;
      sprintf(pos,"<text x=\"%.5g\" y=\"%.5g\" transform=\"scale(1,%.5g)\" style=\"font-size:%.5gpt; text-anchor:middle;\">%.5g</text>\n",xmax+0.3*x_scale,(ymax+ymin-y+0.1*y_scale)/ratio,ratio,fontscale,y);
      pos = buffer+strlen(buffer);
  ...
  static string svg_text(gen A, string legende, svg_attribut attr,double xmin,double xmax,double ymin,double ymax,GIAC_CONTEXT){
    double right_space=(xmax-xmin)/50;
    double text_size=0.5*(ymax-ymin)/10;
    if (legende=="")
      return legende;
    // remove " from legende
    ...
    return "<text  fill=\"" +color_string(attr)+"\"  x=\""+x.print(contextptr)+"\" y=\""+y.print(contextptr)
      +"\" transform=\"translate(0,"+print_DOUBLE_(ymin+ymax,contextptr)+") scale(1,-1)\" style=\"font-size:"+print_DOUBLE_(text_size,contextptr)+"pt; text-anchor:end;\">"
      +legende
      +"</text>\n";                                      
  }

  static double geo_thickness(double xmin,double xmax,double ymin,double ymax){
    return std::sqrt((xmax-xmin)*(ymax-ymin));
  }
  
  static string svg_circle(gen diameter0, gen diameter1, svg_attribut attr, string legende,double xmin,double xmax,double ymin,double ymax,GIAC_CONTEXT){
    string s="";
    ...
    s=  "<circle stroke=\""+color_string(attr)+"\"  stroke-width=\""+thickness.print(contextptr)+"\" fill=\"none\" cx=\""
      +re(center,contextptr).print(contextptr)+"\" cy=\""
      +im(center,contextptr).print(contextptr)+"\" r=\""
      +radius.print(contextptr)
      +"\" />\n";
    if (legende!="")
      s+=svg_text(evalf(center+radius,1,contextptr),legende,attr,xmin,xmax,ymin,ymax,contextptr);
    return s;
  }
  ...
  static string svg_polyline(gen g,svg_attribut attr, string name,double xmin,double xmax,double ymin,double ymax,GIAC_CONTEXT){
    ...
    vecteur v=*(g._VECTptr);
    if (v[0]==v[v.size()-1])
      s= "<polygon  stroke-width=\""+thickness.print(contextptr)+"\" stroke=\""+color_string(attr)+"\" fill=\"none\" points=\"";
    else
      s= "<polyline  stroke-width=\""+thickness.print(contextptr)+"\" stroke=\""+color_string(attr)+"\" fill=\"none\" points=\"";
  ...
  static string svg_bezier_curve(gen g, svg_attribut attr, string legende,double xmin,double xmax,double ymin,double ymax,GIAC_CONTEXT){ 
    ...
    int i;
    gen thickness(geo_thickness(xmin,xmax,ymin,ymax)/svg_epaisseur1*attr.width);
    string s= "<path  stroke-width=\""+thickness.print(contextptr)+"\" stroke=\""+color_string(attr)+"\" fill=\"none\" d=\"M";
  ...
  static string symbolic2svg(const symbolic & mys,double xmin,double xmax,double ymin,double ymax,GIAC_CONTEXT){ 
	...
	if (figure.sommet==at_cercle ){
	  ...
	  return svg_circle(diametre[0],diametre[1], attr, name,xmin,xmax,ymin,ymax,contextptr);
	}
