  ...
  gen simplifier(const gen & g,GIAC_CONTEXT){
	...
	if (wi.is_symb_of_sommet(at_pow) && f.type==_VECT && f._VECTptr->size()==2 && f._VECTptr->back().type==_FRAC){
	  ...
	  if (d.type==_INT_){
	    ...
	    if (zn.type==_ZINT && mpz_sizeinbase(*zn._ZINTptr,2)>128){
	      gen f(zn);
	      pos=is_positive(zn,contextptr);
	      if (!pos) f=-zn;
	      // detect perfect square
	      if (mpz_perfect_power_p(*f._ZINTptr)){
		int nbits=mpz_sizeinbase(*zn._ZINTptr,2);
		...
		if (pow(h,d,contextptr)==f){
	      ...
	      if (!done){
		// trial division only
		pfacprem(f,false,contextptr);
		zint2simpldoublpos(zn/f,a,b,pos,d.val,contextptr);
		a=a*f;
	      }
	    }
	    else
  ...
  */
  ...
  gen _findhelp(const gen & g,GIAC_CONTEXT){
	...
	argss=argss.substr(1,argss.size()-2);
      ...
      return string2gen(string(howto)+"<br>"+argss+'('+string(syntax)+")<br>"+string(related)+"<br>"+string(examples),false);
#endif
#ifdef NSPIRE
      if (argss.size()>2 && argss[0]=='\'' && argss[argss.size()-1]=='\'')
	argss=argss.substr(1,argss.size()-2);
